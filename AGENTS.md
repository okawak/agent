...
- **運用プロセス（ブランチ → docs 要件定義 → PR/レビュー → 実装 → CI）** を標準化する。
...
## 4. 開発プロセス（ブランチ / docs 運用）

### 4.1 要件定義（docs/ 記録）
- **開発開始前**に main からブランチを作成し、各機能の要件を **docs/** 配下の適切なパスに日本語で記述する。
- 要件定義ドキュメントには以下を含める：目的／受入基準／影響範囲（UI/API/ツール/ログ/テスト）／非機能（性能・セキュリティ）／検証方法。
- 仕様変更が生じた場合はドキュメントを更新し、履歴が追えるようコミットメッセージに変更理由を明記する。

### 4.2 ブランチ戦略と PR
- 作業は **main からブランチ作成**（例：feature/xxx, fix/yyy）。初手は要件定義ドキュメントの PR を作成し、レビューを通過してから実装に進む。
- `git`コマンドを使用して、pushする部分まで行う
- 実装フェーズでは、要件定義ドキュメントへのリンク（ファイルパスと該当セクション）を PR 説明に含める。
- **マージ前レビュー必須**：
  - ユニットテスト（TDD）の追加・更新。
...
- [ ] 要件定義ドキュメント（docs/）が最新化され、PR から参照されている
