...
## 受入基準
- `app/` 配下にトップページ（仮ルート：`/playground`）の構造が定義され、UI は SSR/サーバーコンポーネントで実装されている。
- ページ内に以下を最低限含める。
  - Mastra Playground の目的と利用時の注意事項。
  - 利用可能（または計画中）の検証機能カテゴリと各ページへのリンク枠（プレースホルダー可）。
...
- UI は shadcn/ui か MUI のいずれか、または両者の組合せを使っても良いが、採用理由を docs に追記できるよう構成案を残す。
...
- **テスト**: UI レンダリングテストの追加、リンク存在検証、サーバーコンポーネントが client bundle に含まれないことの確認手段を整理する。

## 非機能要件
### 性能
- ページ初期表示はサーバーコンポーネントで構成し、クライアントバンドルを生成しない。
...
- ブラウザで `/playground` を開き、リンク群が SSR のみでレンダリングされていることを開発者ツールの Network/JS プロファイルで確認する。
- 生成される HTML 内にクレデンシャルやサーバー専用モジュールが含まれていないことをビルド出力で確認する。
