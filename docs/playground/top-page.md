...
## 実装メモ（2025-11-03）
- `/playground` はサーバーコンポーネント構成による SSR を採用。CSR / SSG は必要時に再検討する。
- UI は Tailwind ベースでモダンなカードレイアウトを実装し、追加予定の各ページをカードで示すプレースホルダーを配置した。
- セクション定義は `app/playground/data.ts` の zod スキーマで検証し、ユニットテストで構造と ID 重複を監視する。
